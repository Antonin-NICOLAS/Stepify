...
const mockFriends = [
    { id: 1, username: "sophie_fit", firstName: "Sophie", lastName: "Martin", avatarUrl: "/placeholder.svg?height=50&width=50", weeklySteps: 87500 },
    { id: 2, username: "thomas_runner", firstName: "Thomas", lastName: "Bernard", avatarUrl: "/placeholder.svg?height=50&width=50", weeklySteps: 75200 },
    { id: 3, username: "johndoe", firstName: "John", lastName: "Doe", avatarUrl: "/placeholder.svg?height=50&width=50", weeklySteps: 68700 },
    { id: 4, username: "emma_walk", firstName: "Emma", lastName: "Petit", avatarUrl: "/placeholder.svg?height=50&width=50", weeklySteps: 62300 },
    { id: 5, username: "lucas_step", firstName: "Lucas", lastName: "Dubois", avatarUrl: "/placeholder.svg?height=50&width=50", weeklySteps: 58900 }
]
...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        if (progress >= 100) {
            ...
            message = `Super ! Tu as atteint ${Math.round(progress)}% de ton objectif 👟`
        } else if (progress >= 50) {
            message = `Continue comme ça ! Tu es à ${Math.round(progress)}% de ton objectif 🚶`
        } else if (progress >= 25) {
            message = `Bon début ! Tu as fait ${Math.round(progress)}% de ton objectif 👣`
        } else {
            ...
            <div className="dashboard-content">
                ...
                <section className="dashboard-section daily-summary">
                    ...
                    <div className="daily-progress-container">
                        <div className="daily-progress-info">
                            <div className="daily-progress-text">
                                <span className="daily-steps">{formatNumber(mockDailyStats.steps)}</span>
                                <span className="daily-goal">/ {formatNumber(user?.dailyGoal ?? 10000)} pas</span>
                            </div>
                    ...
                    <div className="daily-stats">
                        ...
                        <div className="daily-stat-card">
                            ...
                            <div className="daily-stat-content">
                                <div className="daily-stat-value">{mockDailyStats.activeTime}</div>
                        ...
                        <div className="daily-stat-card">
                            ...
                            <div className="daily-stat-content">
                                <div className="daily-stat-value">{mockDailyStats.calories}</div>
                    ...
                    <div className="activity-mode-streak">
                        <div className="activity-mode">
                            ...
                            <div className="activity-mode-text">
                                Mode principal: <span>{mockDailyStats.mode === "walk" ? "Marche" : mockDailyStats.mode === "run" ? "Course" : "Vélo"}</span>
                            </div>
                        </div>
                        ...
                        <ResponsiveContainer width="100%" height={300}>
                            {timeRange === "week" ? (
                                <BarChart data={chartData}>
                                    ...
                                    <Bar
                                        dataKey={chartType}
                                        name={chartType === "steps" ? "Pas" : chartType === "distance" ? "Distance (km)" : "Calories"}
                                        fill="var(--Couleur1)"
                                        radius={[4, 4, 0, 0]}
                                    />
                                    {chartType === "steps" && (
                    ...
                    <div className="activity-summary">
                        <div className="activity-summary-item">
                            ...
                            <div className="summary-value">
                                {chartType === "steps"
                                    ? formatNumber(Math.round(chartData.reduce((sum, day) => sum + day[chartType], 0) / chartData.length))
                                    : (chartData.reduce((sum, day) => sum + day[chartType], 0) / chartData.length).toFixed(1)}
                                {chartType === "steps" ? " pas" : chartType === "distance" ? " km" : " kcal"}
                            </div>
                        </div>
                        <div className="activity-summary-item">
                            ...
                            <div className="summary-value">
                                {chartType === "steps"
                                    ? formatNumber(Math.max(...chartData.map(day => day[chartType])))
                                    : Math.max(...chartData.map(day => day[chartType])).toFixed(1)}
                                {chartType === "steps" ? " pas" : chartType === "distance" ? " km" : " kcal"}
                            </div>
                        </div>
                        <div className="activity-summary-item">
                            ...
                            <div className="summary-value">
                                {chartType === "steps"
                                    ? formatNumber(chartData.reduce((sum, day) => sum + day[chartType], 0))
                                    : chartData.reduce((sum, day) => sum + day[chartType], 0).toFixed(1)}
                                {chartType === "steps" ? " pas" : chartType === "distance" ? " km" : " kcal"}
                            </div>
                        </div>
                    </div>
                ...
                <section className="dashboard-section custom-goals">
                    <div className="section-header">
                        ...
                        <button className="add-button" onClick={() => setShowNewGoalForm(true)}>
                            <Plus size={20} />
                        ...
                        <div className="new-goal-form-container">
                            <form className="new-goal-form" onSubmit={handleNewGoalSubmit}>
                                ...
                                <div className="form-actions">
                                    <button type="button" className="cancel-btn" onClick={() => setShowNewGoalForm(false)}>
                                        Annuler
                                    </button>
                                ...
                                <div className="goal-content">
                                    ...
                                    <div className="goal-stats">
                                        <div className="goal-current">
                                            {goal.type === "steps" ? (
                                                `${formatNumber(goal.current)} / ${formatNumber(goal.target)} pas`
                                            ) : goal.type === "distance" ? (
                                                `${goal.current} / ${goal.target} km`
                                            ) : goal.type === "calories" ? (
                                                `${formatNumber(goal.current)} / ${formatNumber(goal.target)} calories`
                                            ) : (
                                        ...
                                        <div className="goal-remaining">
                                            {!goal.isCompleted && (
                                                <>
                                                    {goal.type === "steps" ? (
                                                        `${formatNumber(goal.target - goal.current)} pas restants`
                                                    ) : goal.type === "distance" ? (
                                                        `${goal.target - goal.current} km restants`
                                                    ) : goal.type === "calories" ? (
                                                        `${formatNumber(goal.target - goal.current)} calories restantes`
                                                    ) : (
                    ...
                    <div className="rewards-section">
                        ...
                        <div className="latest-reward">
                            ...
                            <div className="reward-card featured">
                                <div className="reward-icon" style={{ backgroundColor: getRewardColor(mockRewards[2].tier) }}>
                                    <span>{mockRewards[2].iconUrl}</span>
                                </div>
                                <div className="reward-content">
                                    <div className="reward-title">{mockRewards[2].name}</div>
                                    <div className="reward-description">{mockRewards[2].description}</div>
                                    <div className="reward-date">Débloqué le {formatDate(mockRewards[2].unlockedAt)}</div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="rewards-grid">
                                {mockRewards.map(reward => (
                                    <div key={reward.id} className="reward-card small">
                                        <div className="reward-icon" style={{ backgroundColor: getRewardColor(reward.tier) }}>
                                    ...
                                    <div className="challenge-progress">
                                        <div className="challenge-progress-bar">
                                            <div
                                                ...
                                                style={{ width: `${Math.min(100, (challenge.currentSteps / challenge.goalSteps) * 100)}%` }}
                                            ></div>
                                        </div>
                                        <div className="challenge-progress-text">
                                            {Math.round((challenge.currentSteps / challenge.goalSteps) * 100)}%
                                        </div>
                                    </div>
                                    <div className="challenge-stats">
                                        <div className="challenge-steps">
                                            {formatNumber(challenge.currentSteps)} / {formatNumber(challenge.goalSteps)} pas
                                        </div>
                ...
                <section className="dashboard-section friends-leaderboard">
                    ...
                    <div className="leaderboard-container">
                        <div className="leaderboard-podium">
                            <div className="podium-item second-place">
                                <div className="podium-avatar">
                                    <img src={mockFriends[1].avatarUrl || "/placeholder.svg"} alt={mockFriends[1].username} />
                                    <div className="podium-position">2</div>
                                </div>
                                <div className="podium-name">{mockFriends[1].firstName}</div>
                                <div className="podium-steps">{formatNumber(mockFriends[1].weeklySteps)}</div>
                            </div>
                            <div className="podium-item first-place">
                                <div className="podium-avatar">
                                    <img src={mockFriends[0].avatarUrl || "/placeholder.svg"} alt={mockFriends[0].username} />
                                    <div className="podium-position">1</div>
                                </div>
                                <div className="podium-name">{mockFriends[0].firstName}</div>
                                <div className="podium-steps">{formatNumber(mockFriends[0].weeklySteps)}</div>
                            </div>
                            <div className="podium-item third-place">
                                <div className="podium-avatar">
                                    <img src={mockFriends[2].avatarUrl || "/placeholder.svg"} alt={mockFriends[2].username} />
                                    <div className="podium-position">3</div>
                                </div>
                                <div className="podium-name">{mockFriends[2].firstName}</div>
                                <div className="podium-steps">{formatNumber(mockFriends[2].weeklySteps)}</div>
                            </div>
                        </div>
                            ...
                            {mockFriends.slice(3).map((friend, index) => (
                                <div key={friend.id} className="leaderboard-item">
                                    ...
                                    <div className="leaderboard-user">
                                        <img src={friend.avatarUrl || "/placeholder.svg"} alt={friend.username} className="leaderboard-avatar" />
                                        <div className="leaderboard-name">
                                            <div>{friend.firstName} {friend.lastName}</div>
                                            <div className="leaderboard-username">@{friend.username}</div>
                                        </div>
                                    </div>
                                    <div className="leaderboard-steps">{formatNumber(friend.weeklySteps)}</div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="your-position">
                            ...
                            <div className="your-position-rank">3<span>e</span></div>
