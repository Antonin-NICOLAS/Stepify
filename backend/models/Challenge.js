...
const challengeSchema = new Schema({
  ...
  endDate: { 
    type: Date,
    validate: [validateDates, 'La date de fin doit être après la date de début']
  },
  ...
  goal: { 
    ...
    required: true, 
    min: 1,
    validate: [validateGoal, 'L\'objectif minimum n\'est pas atteint pour ce type d\'activité']
  },
  time: {
    type: Number,
    min: 1,
  ...
  xpReward: { 
    type: Number, 
    required: true,
    validate: [validateXPReward, 'La récompense XP minimum n\'est pas atteinte pour ce niveau de difficulté']
  },
  difficulty: {
    type: String,
