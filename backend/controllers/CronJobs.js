...
const saveRank = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "[CRON] Historique du classement enregistré" });
  } catch (error) {
    res
      .status(500)
      .json({ error: "[CRON] Erreur enregistrement du classement" });
  }
};
...
const DeleteExpiredNotifications = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({
        message: "[CRON] Suppression des notifications expirées terminée",
      });
  } catch (error) {
    return res
      .status(500)
      .json({ error: "[CRON] Error deleting notifications" });
  }
};
...
const RewardUpdates = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({
        message: "[CRON] Mise à jour des récompenses quotidiennes terminée",
      });
  } catch (error) {
    return res
      .status(500)
      .json({
        error:
          "[CRON] Erreur lors de la mise à jour des récompenses quotidiennes",
      });
  }
};
...
const DeleteLonelyChallenges = async (req, res) => {
  try {
    ...
    return res
      ...
      .json({
        message:
          "[CRON] Suppression des challenges orphelins et notif associées terminée",
      });
  } catch (error) {
    return res
      .status(500)
      .json({
        erreur: "[CRON] Erreur lors de la suppression des challenges orphelins",
      });
  }
};
...
const DeleteLonelyChallengesInternal = async () => {
  try {
    ...
    console.log(
      `[CRON] Deleted ${deletedCount.deletedCount} lonely challenges`
    );
  } catch (error) {
    ...
    for (const user of users) {
      ...
      if (user.activeSessions.length < originalSessionCount) {
        ...
        totalSessionsDeleted +=
          originalSessionCount - user.activeSessions.length;
      }
    }
