...
const getFriends = async (req, res) => {
    ...
    try {
        const user = await UserModel.findById(userId).populate('friends.userId', 'username avatarUrl firstName lastName status totalSteps totalDistance totalXP');
        if (!user) {
...
const getFriendRequests = async (req, res) => {
  ...
  try {
    ...
    const sentRequests = await Notification.find({sender: userId, type: 'friend_request'})
    .populate('recipient', 'firstName lastName username avatarUrl')

    return sendLocalizedSuccess(res, null, {}, { requests: user.friendRequests, sent: sentRequests });
  } catch (error) {
