...
const addCustomGoalFromReward = async (req, res) => {
  try {
    ...
    await createSystemNotification({
      ...
      content: {
        en: `New goal added: ${
          reward.name[user.languagePreference] || reward.name.en
        }`,
        fr: `Nouvel objectif ajouté : ${
          reward.name[user.languagePreference] || reward.name.fr
        }`,
        es: `Nuevo objetivo añadido: ${
          reward.name[user.languagePreference] || reward.name.es
        }`,
        de: `Neues Ziel hinzugefügt: ${
          reward.name[user.languagePreference] || reward.name.de
        }`,
      },
    })
